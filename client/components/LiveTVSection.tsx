// File: client/components/LiveTVSection.tsx
'use client';

import React, { useState, useEffect, useRef } from 'react';
import Hls from 'hls.js';
import { Tv, Play, Pause, Volume2, VolumeX, RefreshCw, Radio, Shield, AlertCircle, Sparkles, Wifi } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, () => {
        ...
        if (isPlaying) {
          video.play().catch(() => {
            // Auto-play muted fallback if blocked by browser
            video.muted = true;
            setIsMuted(true);
        ...
        if (data.fatal) {
          switch (data.type) {
            case Hls.ErrorTypes.NETWORK_ERROR:
              console.warn('Network error encountered, trying proxy/fallback...');
              hls.startLoad();
              break;
